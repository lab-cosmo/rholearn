...
def get_global_basis_set(
    ...
    return mts.Labels(
        ...
        values=_dispatch.int_array(
            [[l, a, nmax] for (l, a), nmax in global_basis.items()], backend=backend
        ),
    )
...
def coeff_vector_ndarray_to_dict(
    ...
    """
    Convert a flat array of basis function coefficients (or projections) to a
    dictionary of the form:

    Keys: tuples of (l, symbol)
    Blocks: dict of the form {i: array} where array is the coefficient array for
        the atom ``i``.
    """
    # Define some useful variables
    symbols = system.get_symbols(frame)
    tot_coeffs = len(coeff_vector)
...
def coeff_vector_ndarray_to_tensormap(
    ...
    :param coeff_vector: np.ndarray of shape (N,), where N is the number of basis functions
        the electron density is expanded onto.
    ...
    :param structure_idx: int, the index of the structure in the overall dataset. If
        ...
        not contain an index for the structure, i.e. the sample names will just be
    ...
    keys = mts.Labels(
        ...
        values=np.array(
            [
                [l, 1, system.atomic_symbol_to_atomic_number(symbol)]
                for l, symbol in coeff_dict.keys()
            ]
        ),
    )
    ...
    blocks = []
    for l, _, center_type in keys:
        symbol = system.atomic_number_to_atomic_symbol(center_type)
        raw_block = coeff_dict[(l, symbol)]
...
def overlap_matrix_ndarray_to_tensormap(
    ...
    hierarchically, ["o3_lambda", "n", "o3_mu"] for each in a pair of basis functions.
    """
    # Parse ndarray -> dict
...
def coeff_vector_to_sparse_by_center_type(
    ...
    indices, i.e. with "o3_lambda" being the outer loop and "o3_mu" being the inner
    loop.
    """
    if backend == "numpy":
        mts = metatensor
...
def coeff_vector_blocks_to_flat(
    ...
    for atom_i, symbol in enumerate(system.get_symbols(frame)):
        for l in range(lmax[symbol] + 1):
            coeffs = np.append(coeffs, coeff_dict[(l, symbol)][atom_i])
...
def labels_basis_def_to_lmax_nmax_dicts(
    ...
    """
    Converts a metatensor Labels object containing the basis set definition, of the
    form:
...
def test_coeff_vector_conversion(
    ...
    flat_index = _get_flat_index(species_symbols, lmax, nmax, i, l, n, m)
    ...
    if structure_idx is None:
        s_idx = tm_block.samples.position(
            metatensor.Labels(names=["atom"], values=np.array([[i]]))[0]
        )
    else:
        s_idx = tm_block.samples.position(
            metatensor.Labels(
    ...
    c_idx = tm_block.components[0].position(
        metatensor.Labels(names=["o3_mu"], values=np.array([[m]]))[0]
    )
