...
Until this module is made available in a public library, this is a copy of:
https://github.com/Luthaf/rascaline/blob/master/python/rascaline/tests/utils/rotations.py
"""
...
def transform_frame_so3(frame: Frame, angles: Sequence[float]) -> Frame:
    """
    Transforms the positions and cell coordinates of an :py:class:`chemfiles.Frame` by a
    SO(3) rigid rotation.
    """
...
def transform_frame_o3(frame: Frame, angles: Sequence[float]) -> Frame:
    """
    Transforms the positions and cell coordinates of a :py:class:`chemfiles.Frame` by an
    O(3) rotation. This involves a rigid SO(3) rotation of the positions and cell
    according to the Euler `angles`, then an inversion by multiplying just the positions
    by -1.
    """
    new_frame = frame.copy()
...
class WignerDReal:
    ...
    def transform_flat_vector_so3(
        ...
        structure. ``i`` takes values 0 to N (** exclusive **), where N is the number of
        atoms in the structure.

        2. Loop over spherical harmonics channel (index ``l``) for each atom.
        ``l`` takes values from 0 to ``lmax[a] + 1`` (** exclusive **), where ``a`` is
        the chemical species of atom ``i``, given by the chemical symbol at the ``i``th
        position of ``symbol_list``.

        3. Loop over radial channel (index ``n``) for each atom ``i`` and
        spherical harmonics channel ``l`` combination. ``n`` takes values from 0 to
        ``nmax[(a, l)]`` (** exclusive **).

        4. Loop over spherical harmonics component (index ``m``) for each atom.
        ``m`` takes values from ``-l`` to ``l`` (** inclusive **).

        :param frame: the atomic structure in :py:class:`chemfiles.Frame` format for
            which the coefficients are defined.
        ...
        curr_idx = 0
        for symbol in system.get_symbols(frame):
    ...
    def transform_flat_vector_o3(
        ...
        """
        A mirror of `transform_flat_vector_so3`, but with parity change of
        coefficients for l odd to account for inversion O(3) transformation.
        """
        # Initialize empty vector for storing the rotated ISCs
        rot_vect = np.empty_like(coeffs)
