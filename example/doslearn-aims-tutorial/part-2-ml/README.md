...
The training script trains a machine learning model based on the output of FHI-AIMS generated by [part-1-dft](../part-1-dft/). The script first generates SOAP Power spectrum descriptors based on structures in the .xyz file defined in the XYZ field of [dft-options.yaml](../part-1-dft/dft-options.yaml). Then, the script parses the eigenenergies obtained in the Final_KS_eigenvalues.dat output file of each structure and generate splines within a specified energy cutoff, and energy reference, in [ml-options.yaml](./ml-options.yaml). Then the script performs batched gradient descent to train a model, whose architecture is defined by the `HIDDEN_LAYER_WIDTHS` parameter in the yaml file. 
